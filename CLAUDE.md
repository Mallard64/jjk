...
- **Proxy smoothness:** this Fusion build's `NetworkTransform` only interpolates the *root* (no interpolation-target field), which fights the authority's physics-driven movement — so root interpolation is left **off** (`DisableSharedModeInterpolation` on the player prefab's NetworkTransform) and proxies snap between network ticks. `ProxyRootSmoother` (player root) fixes the resulting stepping by easing the whole proxy root toward its networked position each `LateUpdate` — sprite/shadow/bars/collider move together, no per-part desync. Strictly **proxy-only** (no-op on the authority and offline, so it never touches local responsiveness) and adds no network traffic. Tunables: `smoothTime`, `snapDistance`. (An earlier sprite-only version desynced the shadow and delayed hits; raising the client send rate made it worse — on one PC the extra packet load just taxes the shared CPU — both reverted.)
...
6. When a player hits 0 HP they play their death animation and both players freeze.
   ...
   - **Online** (the `Arena` scene's `GameLauncher`/Fusion path): a **best-of-5 match** (first to 3 round wins). Each peer runs the round/match flow in `FusionPlayerSync` — it watches every player it knows (own + opponent proxies) and, on any death, freezes the local player (static `RoundResetting`, read by `FusionPlayerMovement`/`FusionPlayerCombat`), **awards the round to the survivor** (each winner's authority increments its own replicated `NetworkedRoundWins`; the loser's routine skips it, so exactly one side scores per round), holds briefly on the death pose (`deathLinger`), **fades to white** (`ScreenBlackout.FadeTo(…, Color.white)`), and shows the **ROUND END** screen (`NetworkMatchUI`). If neither side has reached `roundsToWin` it respawns its own player at its spawn and the next round begins; once someone hits `roundsToWin` it shows **VICTORY / DEFEAT** and calls `Runner.Shutdown()` so **both peers disconnect and the arena closes**. Death/respawn animation replicates to proxies through `NetworkedDead`; the running score is shown by `NetworkMatchUI` (OnGUI).
