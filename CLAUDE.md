...
- **Only movement is predicted. Combat is not, and must not be.** `FusionPlayerCombat`, `PlayerRoll`, the attack controllers and the domain all keep non-networked local state (coroutines, phase timers, cooldowns) that a client cannot reproduce or rewind, so they stay gated on `HasStateAuthority` and reach clients via RPC. **A predicted tick must be reproducible from `[Networked]` state + input, and nothing else.** That is why the movement gates (`VelocityLocked` / `MoveBlocked` / `SpeedScale`) are `[Networked]`: they are resolved from the controllers on the server and published, so the predicting client reproduces the same motion during attacks, rolls and hitstun instead of mispredicting through them.
...
2. Override `OnDomainStarted()` / `OnDomainEnded()` for the **bonus** (e.g. set `MoveSpeedMultiplier`) and any extra VFX/SFX. Optional `domainVfx` GameObject + `audioSource`/`activateSfx` fields are toggled/played by the base.
