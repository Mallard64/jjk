...
/// <summary>
/// Resonance resource. Techniques call TrySpend/Drain/Restore; they do NOT own this
/// directly. Passive regen can be suppressed for a window via SuppressRegenForAction so actively
/// committing to an attack/roll pauses the meter from ticking back up. Actions that cost RES check
...
public class Resonance : MonoBehaviour
{
    ...
    /// <summary>While true, passive regen is suppressed (set each frame by the null field system: any active
    /// null field or this player's null field burnout freezes their RES so they fight on a fixed budget).</summary>
    ...
    void Update()
    {
        ...
        if (RegenBlocked) return;  // null field / burnout freezes the meter
