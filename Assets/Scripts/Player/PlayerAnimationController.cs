...
public class PlayerAnimationController : MonoBehaviour
{
    ...
    /// <summary>Play the null field activation pose (direction-less single clip, like roll). Locked
    /// like an attack so movement doesn't override it during the startup freeze; RefreshMovementState
    /// (called when the null field opens) clears it.</summary>
    public void PlayNullField()
    {
        _attackLocked = true;
        SetAnimatorSpeed(1f);
        // State name is still "domain": the animator is generated by the Aseprite importer from tags
        // authored inside the .aseprite binary, so this one can only be renamed in Aseprite itself.
